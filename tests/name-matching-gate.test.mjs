...
test("the gate loads every bank file the app loads", () => {
  ...
  /* Lessons, diagnoses and coverage carry no option sets, so the gate does not need
     them; every file that DOES define questions must be present.

     t6_coverage.js is generated by tools/measure-syllabus-coverage.mjs --emit and holds
     only per-subject and per-module counts — no stems, no options, nothing the R3
     name-matching measurement can read. It is excluded for the same reason as lessons
     and diagnoses, not as a way round this guard: the guard exists because loading the
     catalog without t6_brgsa.js silently yields 48 concepts instead of 64, and a file
     with no questions in it cannot cause that failure. */
  const questionFiles = shipped.filter((file) => !/t6_(lessons|diagnoses|coverage)\.js$/.test(file));
